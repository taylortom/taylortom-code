/**
 * Represents a stacked task popup
 */
...
(function(){
     ...
     * @param anchorPoint
     */
    var StackedTaskPopupView = function(models,anchorPoint){

        // console.log('StackedTaskPopupView.constructor: ' + model.getTitle() + ' (' + anchorPoint.x + ',' + anchorPoint.y + ')');
        this.initialize(models, anchorPoint);
    };

    // set the 'class'
    var p = StackedTaskPopupView.prototype = new createjs.Container();
    ...
    p.initialize = function(models, anchorPoint){
        ...
        // console.log('StackedTaskPopupView.initialize: ' + this.openPopup);
    ...
    p.attachDisplays = function(){
        ...
        for(var i = 0; i < this._models.length; i++){
        	...
        	var display = new TaskPopupDisplay(this._models[i]);
    ...
    p.drawScreenlock = function(){
        ...
        // console.log('StackedTaskPopupView.drawScreenlock: ' + stage.width + "x" + stage.height);
    ...
    p.drawBackground = function(){
        ...
        // console.log('StackedTaskPopupView.drawBackground: ' + this._offset.x + ',' + this._offset.y);
    ...
    p.positionComponents = function(){
        ...
        this._bg.y = (this._alignPopupBottom) ? Config.getStageHeight()-(this._bg.height + BOTTOM_PADDING) : this._offset.y-(this._display.height/2);
    ...
    p.checkIsOnscreen = function() {
        ...
        if(this._drawBackwards != drawBackwards) {
            ...
            {
            	this._offset.x = (viewX + Config.getStageWidth());// - (10 + this._callout.size);
            }
            changed = true;
        }
        else if(!isWithinScreenBounds(this._offset)) {

            this._offset.x = (viewX + (10+this._callout.size));
            changed = true;
        }
        ...
        if(bottomLeft.y > (Config.getStageHeight() - BOTTOM_PADDING)) {
    ...
    window.StackedTaskPopupView = StackedTaskPopupView;
}());
